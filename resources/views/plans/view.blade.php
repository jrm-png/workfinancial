...
<!-- <div id="viewModal" style="display:none; position:fixed; z-index:9999; left:0; top:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.7); backdrop-filter: blur(8px); overflow-y: auto;">
    ...
    ```
        <div style="padding: 20px 40px; border-bottom: 2px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; background: white; position: sticky; top: 0; z-index: 100;">
            <div>
                <span style="font-weight: 800; color: #2563eb; letter-spacing: 1px; font-size: 12px; text-transform: uppercase; display: block; margin-bottom: 4px;">
                    Viewing Mode
                </span>

                <h2 style="margin:0; color:#1e293b; font-size: 24px; font-weight: 900;" id="modalTitleHeading">
                    Work & Financial Plan
                </h2>
            </div>
            ...
            <div style="display: flex; gap: 15px; align-items: center;">
                ...
                @php
                    ...
                    $hasModifierAccess = in_array($userRole, [
                        'admin',
                        'MONITOR',
                        'APPROVER',
                        'REVIEWER',
                ...
                @if($hasModifierAccess)
                    ...
                    <div id="statusActionWrapper"
                        style="display: flex; align-items: center; gap: 10px; background: #f1f5f9; padding: 8px 15px; border-radius: 12px; border: 1px solid #cbd5e1;">
                        ...
                        <select id="adminStatusSelect"
                                style="padding: 6px 12px; border-radius: 8px; border: 1px solid #cbd5e1; font-weight: 800; font-size: 13px; color: #1e293b; background: white;">
                        ...
                        <button id="adminStatusUpdateButton"
                                onclick="updateStatusAndComment()"
                                style="background:#2563eb; color:white; border:none; padding:8px 16px; border-radius:8px; font-size:13px; font-weight:700; cursor:pointer;">
                            Update
                        </button>
                ...
                <div id="readOnlyStatusLabelWrapper"
                    style="display:none; align-items:center; gap:8px; background:#f8fafc; padding:8px 15px; border-radius:12px; border:1px solid #e2e8f0;">

                    <label style="font-size:13px; font-weight:700; color:#64748b;">
                        STATUS:
                    </label>

                    <span id="preparerStatusLabel"
                        style="font-weight:800; font-size:13px; color:#1e293b;">
                    </span>

                </div>

                <button onclick="window.print()"
                        style="background:#64748b; color:white; border:none; padding:10px 20px; border-radius:10px; cursor:pointer; font-weight:600;">
                    <i class="fas fa-print"></i> Print
                </button>

                <button onclick="closeModal()"
                        style="background:white; border:2px solid #e2e8f0; padding:10px 20px; border-radius:10px; cursor:pointer; font-weight:700; color:#475569;">
                    ✕ Close
                </button>
        ...
        <div style="padding:40px;">
            ...
            <div style="margin-bottom:30px; display:flex; justify-content:space-between; align-items:flex-end;">
                ...
                <h1 id="modalMainTitle"
                    style="border-left:6px solid #2563eb; padding-left:20px; font-size:32px; font-weight:800; margin:0; color:#0f172a;">
                    Plan Details
                </h1>

                <div id="modalSubtitle"
                    style="text-align:right; font-weight:700; color:#64748b; font-size:16px; background:white; padding:10px 20px; border-radius:10px; border:1px solid #e2e8f0;">
                </div>
            ...
            <div style="background:white; padding:30px; border-radius:16px; border:1px solid #e2e8f0; box-shadow:0 4px 6px -1px rgba(0,0,0,0.05); margin-top:20px;">
                ...
                <label style="font-weight:800; font-size:14px; text-transform:uppercase; color:#475569; display:block; margin-bottom:15px; letter-spacing:0.5px;">
                    Admin / Evaluator Feedback
                </label>

                <textarea id="adminCommentBox"
...
<script>
    ...
    */
    ...
    function showDetails(id, type = 'workplan') {
        ...
        fetch(`/workplan/unified/${id}`)
            ...
            .then(data => {
                ...
                /*
                |--------------------------------------------------------------------------
                | Select the correct main record
                |--------------------------------------------------------------------------
                */
                ...
                if (!firstRecord) {
                    return alert("No records discovered.");
                }

                const currentStatus =
                    (firstRecord.status || 'PENDING').toUpperCase();
                ...
                */
                ...
                if (['admin', 'MONITOR'].includes(currentAuthRole)) {
                    ...
                    currentAuthRole === 'REVIEWER' &&
                    ['PENDING', 'FOR REVIEW', 'FOR REVISION'].includes(currentStatus)
                ) {

                    isActionableState = true;

                } else if (
                    currentAuthRole === 'DEPARTMENT MANAGER' &&
                    ['FOR REVIEW', 'FOR SUBMISSION TO FINANCE'].includes(currentStatus)
                ) {

                    isActionableState = true;

                } else if (
                    currentAuthRole === 'FINANCE' &&
                    ['FOR SUBMISSION TO FINANCE', 'APPROVED'].includes(currentStatus)
                ) {
                ...
                */
                ...
                const actionContainer =
                    document.getElementById('statusActionWrapper');

                const labelContainer =
                    document.getElementById('readOnlyStatusLabelWrapper');

                const commentTextArea =
                    document.getElementById('adminCommentBox');

                const statusLabel =
                    document.getElementById('preparerStatusLabel');
                ...
                */
                ...
                document.getElementById('currentViewingFormId').value =
                    firstRecord.form_id;

                document.getElementById('modalSubtitle').innerText =
                    `${firstRecord.r_center || 'RC'} | Planning Year: ${firstRecord.year || '2026'}`;

                document.getElementById('adminCommentBox').value =
                    firstRecord.comment || '';
                ...
                */
                ...
                if (type === 'financial') {
                    ...
                    document.getElementById('modalTitleHeading').innerText =
                        'Financial Plan';
                        ...
                        financialHtml += `
                            ...
                            <div class="view-card">
                                ...
                                ">
                                    ...
                                    <div>
                                        <label class="view-label">Funds</label>
                                    ...
                                    <div>
                                        <label class="view-label">Expense Class</label>
                                        <div class="view-value">
                                            ${f.expense_class || '-'}
                                        </div>
                                    </div>
                                ...
                                ">
                                    ...
                                    <div>
                                        <label class="view-label">Project</label>
                                        <div class="view-value">
                                            ${f.projects || '-'}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <label class="view-label">Activity</label>
                                        <div class="view-value">
                                            ${f.activity || '-'}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <label class="view-label">Account Title</label>
                                        <div class="view-value">
                                            ${f.account_title || '-'}
                                        </div>
                                    </div>
                                ...
                                <div style="margin-bottom:20px;">
                                    ...
                                    <div class="view-value"
                                        ...
                                        ${f.description || 'No description specified.'}
                                ...
                                ">
                                    ...
                                    <div>
                                        <span class="view-label">Q1 Amount</span>
                                        <div class="view-value" style="font-weight:700;">
                                            ₱${Number(f.q1 || 0).toLocaleString(undefined, {
                                    ...
                                    <div>
                                        <span class="view-label">Q2 Amount</span>
                                        <div class="view-value" style="font-weight:700;">
                                            ₱${Number(f.q2 || 0).toLocaleString(undefined, {
                                                minimumFractionDigits: 2
                                            })}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <span class="view-label">Q3 Amount</span>
                                        <div class="view-value" style="font-weight:700;">
                                            ₱${Number(f.q3 || 0).toLocaleString(undefined, {
                                                minimumFractionDigits: 2
                                            })}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <span class="view-label">Q4 Amount</span>
                                        <div class="view-value" style="font-weight:700;">
                                            ₱${Number(f.q4 || 0).toLocaleString(undefined, {
                                                minimumFractionDigits: 2
                                            })}
                                        </div>
                                    </div>
                                    ...
                                    ">
                                        ...
                                        <span class="view-label" style="color:#059669;">
                                            Row Total
                                        </span>

                                        <div class="view-value"
                                            style="font-weight:900; font-size:15px; color:#059669;">
                    ...
                    workPlans.forEach((wp, index) => {
                        ...
                        if (wp.attachments) {
                            ...
                            try {
                                ...
                                const files =
                                    typeof wp.attachments === 'string'
                                        ? JSON.parse(wp.attachments)
                                        : wp.attachments;
                                ...
                                files.forEach(path => {
                                    ...
                                    const secureViewUrl =
                                        `/workplan/view-attachment?path=${encodeURIComponent(path)}`;

                                    attachmentHtml += `

                                        <a href="${secureViewUrl}"
                                        target="_blank"
                            ...
                            <div class="view-card">
                                ...
                                <div style="margin-bottom:35px;">
                                    ...
                                    <div class="target-grid">
                                        ...
                                        <div class="target-item">
                                            <span class="view-label">Q1</span>
                                            <div class="view-value">
                                                ${wp.q1 || 0}${wp.unit_type === 'percent' ? '%' : ''}
                                            </div>
                                        </div>

                                        <div class="target-item">
                                            <span class="view-label">Q2</span>
                                            <div class="view-value">
                                                ${wp.q2 || 0}${wp.unit_type === 'percent' ? '%' : ''}
                                            </div>
                                        </div>

                                        <div class="target-item">
                                            <span class="view-label">Q3</span>
                                            <div class="view-value">
                                                ${wp.q3 || 0}${wp.unit_type === 'percent' ? '%' : ''}
                                            </div>
                                        </div>

                                        <div class="target-item">
                                            <span class="view-label">Q4</span>
                                            <div class="view-value">
                                                ${wp.q4 || 0}${wp.unit_type === 'percent' ? '%' : ''}
                                            </div>
                                        </div>
                                ...
                                ">
                                    ...
                                    ">
                                        ...
                                        Financial Plan for this Initiative

                                    </h4>

                                    ${
                                        wpFinancials.length > 0
                                        ...
                                        : '<p style="color:#94a3b8; font-style:italic; font-size:14px;">No financial rows added.</p>'
                                    }
                                ...
                                ">
                                    ...
                                    ">
                                        ...
                                        ${
                                            attachmentHtml ||
                                            '<span style="color:#fb7185; font-size:13px;">No files attached for this initiative.</span>'
                                        }
    ...
    */
    ...
    function updateStatusAndComment() {
        ...
        if (!selectElement) {
            ...
            return alert(
                "Action unauthorized for your account scope."
            );
        ...
        if (!formId || formId === "undefined") {
            ...
            return alert(
                "Error: Form unique reference is missing."
            );
        ...
        .catch(err => {
            ...
            alert(
                'Process failed during state save.'
            );
